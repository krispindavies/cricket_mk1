...
public:
  // Constructors
  kMatrix(const KOS_INT rows,
          const KOS_INT columns = 1);  // Standard Constructor for (n x m) matricies.
  ...
  KOS_INT get_size_r() const
  {
    return size_r;
  }  // Rows
  KOS_INT get_size_c() const
  {
    return size_c;
  }  // Columns

  // Const data access operator.
  KOS_DEC get_val(KOS_INT row, KOS_INT col) const
  {
...
// Typedef to create a vector type (which is really just a 1d version of the
// matrix class).
...
public:
  ...
  hMatrix(const hMatrix& source)
  {
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 4; j++)
        data[i][j] = source.get_val(i, j);
    GL_Table = NULL;
  }
  ~hMatrix()
  {
  ...
  // The *= operator.
  // This operator does a RIGHT SIDE MULTIPLICATION, ie: [result] =
  // [arg1]*[*this] The purpose of this is to tack on homogeneous transforms in
  // logical order.
  ...
  // Function for transforming an hMatrix into a kMatrix. (this is a ONE WAY
  // operation!)
...
public:
  ...
  hVector(const hVector& source)
  {
    for (int i = 0; i < 4; i++)
      data[i] = source.get_val(i);
  }
  ...
  // Function for transforming an hVector into a kMatrix. (this is a ONE WAY
  // operation!)
  operator kMatrix() const
  {
    kMatrix ret_val(4, 1);
  ...
  // Const data access operator.
  KOS_DEC get_val(KOS_INT row) const
  {
    if (row >= 0 && row < 4)
      return data[row];
    else
...
// Helper functions.
hVector h_Point(const KOS_DEC& arg_x,
                const KOS_DEC& arg_y,
                const KOS_DEC& arg_z);  // Creates an hVector representation of a point.
hVector h_Vec(const KOS_DEC& arg_x,
              const KOS_DEC& arg_y,
              const KOS_DEC& arg_z);  // Creates an hVector representation of a
                                      // vector quantity.
...
hVector h_VRotQuat(const KOS_DEC& Quat1_Pos,
                   const KOS_DEC& Quat2_Pos,
                   const KOS_DEC& Quat3_Pos,
                   const KOS_DEC& Quat4_Pos,
                   const KOS_DEC& Quat1_Vel,
                   const KOS_DEC& Quat2_Vel,
                   const KOS_DEC& Quat3_Vel,
                   const KOS_DEC& Quat4_Vel,
                   const hVector& Pos,
                   const hVector& Vel);
