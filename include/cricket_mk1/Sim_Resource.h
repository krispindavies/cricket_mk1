...
struct Sim_Env_Struct
{
  // Internally used structure for representing a single box within the
  // environment.
  struct Sim_Env_Box
  {
...
public:
  ...
  int getCellCount()
  {
    return m_cells_x * m_cells_y;
  }
  Sim_Env_Box& getFloor(int target)
  {
    return floor_Array[target];
  }
  Sim_Env_Box& getCeiling(int target)
  {
    return ceiling_Array[target];
  }

  // Definition operators allowing you to set the z dimension based on x and y
  // position.
...
public:
  ...
  // Access operator.
  const Contact_Node& operator()(int i) const
  {
    if (i >= 0 && i < m_counter)
  ...
  void addTriangle(const kMath::hVector& vertex_1,
                   const kMath::hVector& vertex_2,
                   const kMath::hVector& vertex_3,
...
struct Sim_Struct
{
  ...
  enum Env_Disp_Mode_Enum
  {
    Environment,
    Map
  };
  enum Crick_Disp_Mode_Enum
  {
    Graphics,
    Contacts
  };
...
public:
  ...
  ~Sim_Struct()
  {
    if (m_image_data != NULL) delete[] m_image_data;
  }

  // Function for updating the simulation until a specific time based on the
  // currently supplied configurations list.
...
private:
  ...
  // Called in order to determine the motor torques for the Cricket. (This
  // determines the current target configuration, and target joint velocities.)
  void Calc_Torques(kSim::Sim_Crick_Struct& In_Crick);

  // A sub-function which determines the force imposed on a single contact node
  // by a single environmental object (box).
  kMath::hVector Calc_Force(kSim::Sim_Env_Struct::Sim_Env_Box curr_floor_node,
                            kMath::hVector contact_node_pos,
                            kMath::hVector contact_node_vel,
                            kMath::hVector contact_node_normal,
                            double contact_area);

  // Called in order to determine the external forces imposed on one body
  // segment of the Cricket by the environment.
