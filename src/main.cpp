...
void display()
{
  ...
  if ((Sim_Struct()->m_SR_image))
  {
    ...
    {
      std::cout << "\n\n\n WARNING! Window insufficient size to render SR4000 "
                   "camera images.\n\n\n";
    ...
    glLoadIdentity();
    gluPerspective(43.6, GLfloat(SR_CAM_PIXEL_WIDTH) / GLfloat(SR_CAM_PIXEL_HEIGHT), SR_CAM_NEAR_PLANE_COORD,
                   SR_CAM_FAR_PLANE_COORD);
    ...
    // Remember to check whether we're displaying the SR4000 view or a user
    // selected view.
    glLoadIdentity();
    ...
    cam_pos = kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
              * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                                 Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
              * kMath::h_Point(MODEL_CAM_Origin_X, MODEL_CAM_Origin_Y, MODEL_CAM_Origin_Z);
    cam_focus = kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                               Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
                * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                                   Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
                * kMath::h_Trans(MODEL_CAM_Origin_X, MODEL_CAM_Origin_Y, MODEL_CAM_Origin_Z)
                * kMath::h_Roty(Sim_Struct()->m_cricket.m_CAM.m_Joint_Angle) * kMath::h_Point(1.0, 0.0, 0.0);
    cam_up = kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                            Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
             * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                                Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
             * kMath::h_Roty(Sim_Struct()->m_cricket.m_CAM.m_Joint_Angle) * kMath::h_Point(0.0, 0.0, 1.0);

    // Define camera position.
    gluLookAt(cam_pos(0), cam_pos(1), cam_pos(2), cam_focus(0), cam_focus(1), cam_focus(2), cam_up(0), cam_up(1),
              cam_up(2));
    ...
    // Remember to cull those objects which are in front of the Cricket so that
    // we can actually see the robot and such.
    for (int i = 0; i < Sim_Struct()->m_environment.getCellCount(); i++)
    {
      // Floor
      double dot = 0;  //(SIMSTRUCT.crick_pos_x() -
                       // SIMSTRUCT.cam_pos_x())*(ENVIRONMENT.get_floor(i).get_x()
                       //- SIMSTRUCT.crick_pos_x()) + (SIMSTRUCT.crick_pos_y() -
                       // SIMSTRUCT.cam_pos_y())*(ENVIRONMENT.get_floor(i).get_y()
                       //- SIMSTRUCT.crick_pos_y()) + (SIMSTRUCT.crick_pos_z() -
                       // SIMSTRUCT.cam_pos_z())*(ENVIRONMENT.get_floor(i).get_z()
                       //- SIMSTRUCT.crick_pos_z());
      if (dot >= 0)
      {
        glPushMatrix();
      ...
      // Ceiling
      dot = 0;  //(SIMSTRUCT.crick_pos_x() -
                // SIMSTRUCT.cam_pos_x())*(ENVIRONMENT.get_ceiling(i).get_x() -
                // SIMSTRUCT.crick_pos_x()) + (SIMSTRUCT.crick_pos_y() -
                // SIMSTRUCT.cam_pos_y())*(ENVIRONMENT.get_ceiling(i).get_y() -
                // SIMSTRUCT.crick_pos_y()) + (SIMSTRUCT.crick_pos_z() -
                // SIMSTRUCT.cam_pos_z())*(ENVIRONMENT.get_ceiling(i).get_z() -
                // SIMSTRUCT.crick_pos_z());
      if (dot >= 0)
      {
        glPushMatrix();
    ...
    if (Sim_Struct()->m_image_data != NULL) delete[] Sim_Struct()->m_image_data;
    Sim_Struct()->m_image_data = new kMath::hVector[176 * 144];

    kMath::hMatrix global_to_cam_trans =
      kMath::h_Roty(-1.0 * Sim_Struct()->m_cricket.m_CAM.m_Joint_Angle)
      * kMath::h_Trans(-MODEL_CAM_Origin_X, -MODEL_CAM_Origin_Y, -MODEL_CAM_Origin_Z)
      * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, -1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                         -1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
                         -1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
      * kMath::h_Trans(-1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_X, -1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                       -1.0 * Sim_Struct()->m_cricket.m_BDY.m_Pos_Z);

    // Transform each pixel into world coordinates and measure distance to
    // camera.
    for (int y = 0; y < 144; y++)
    {
      for (int x = 0; x < 176; x++)
      {
        ...
        // Clip for maximum and minimum range. In either case, camera should
        // output maximum recordable range.
        if (Sim_Struct()->m_image_data[y * 176 + x].mag() < SR_CAM_MIN_RANGE
            || Sim_Struct()->m_image_data[y * 176 + x].mag() > SR_CAM_MAX_RANGE)
          Sim_Struct()->m_image_data[y * 176 + x] =
            kMath::h_Scale(SR_CAM_MAX_RANGE / Sim_Struct()->m_image_data[y * 176 + x].mag())
            * Sim_Struct()->m_image_data[y * 176 + x];
        ...
        Sim_Struct()->m_image_data[y * 176 + x] =
          kMath::h_Scale((1.0 + 0.1 * (std::rand() % 100) / 100.0 - 0.05)) * Sim_Struct()->m_image_data[y * 176 + x];
      }
    }
  }
  ...
  {
    gluPerspective(43.6, GLfloat(Sim_Struct()->m_image_width) / GLfloat(Sim_Struct()->m_image_height),
                   USER_NEAR_PLANE_COORD, USER_FAR_PLANE_COORD);
  }
  else
  {
    gluPerspective(43.6, GLfloat(Sim_Struct()->m_image_width) / GLfloat(1.0), USER_NEAR_PLANE_COORD,
                   USER_FAR_PLANE_COORD);
  }
  ...
  glLightfv(GL_LIGHT0, GL_SPECULAR, white_light);
  gluLookAt(
    Sim_Struct()->m_cricket.m_BDY.m_Pos_X
      - Sim_Struct()->m_display_zoom * std::cos(Sim_Struct()->m_display_pitch) * std::cos(Sim_Struct()->m_display_yaw),
    Sim_Struct()->m_cricket.m_BDY.m_Pos_Y
      - Sim_Struct()->m_display_zoom * std::cos(Sim_Struct()->m_display_pitch) * std::sin(Sim_Struct()->m_display_yaw),
    Sim_Struct()->m_cricket.m_BDY.m_Pos_Z - Sim_Struct()->m_display_zoom * std::sin(Sim_Struct()->m_display_pitch),
    Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y, Sim_Struct()->m_cricket.m_BDY.m_Pos_Z,
    0.0, 0.0, 1.0);
  ...
  switch (Sim_Struct()->m_crick_disp_mode)
  {
    case kSim::Sim_Struct::Graphics:
    {
      // Draw Body.
      {
        ...
        // Draw BDY.
        glPushMatrix();
        kMath::hMatrix Global_Trans =
          kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                         Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4);
      ...
      /*{
              ...
              glPushMatrix();
              kMath::hMatrix Global_Trans =
      kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)*kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_RFS_Origin_X,MODEL_RFS_Origin_Y,MODEL_RFS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_RFS.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
              ...
              // Draw LFS.
              glPushMatrix();
              Global_Trans = kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)*kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_LFS_Origin_X,MODEL_LFS_Origin_Y,MODEL_LFS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_LFS.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFS());
              glPopMatrix();
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_RRS_Origin_X,MODEL_RRS_Origin_Y,MODEL_RRS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_RRS.m_Joint_Angle)*kMath::h_Scale(-1.0,1.0,1.0);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFS());
              glPopMatrix();
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_LRS_Origin_X,MODEL_LRS_Origin_Y,MODEL_LRS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_LRS.m_Joint_Angle)*kMath::h_Scale(-1.0,1.0,1.0);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFS());
              glPopMatrix();
      }
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_RFS_Origin_X,MODEL_RFS_Origin_Y,MODEL_RFS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_RFS.m_Joint_Angle)*kMath::h_Trans(MODEL_RFP_Origin_X,MODEL_RFP_Origin_Y,MODEL_RFP_Origin_Z)*kMath::h_Roty(Sim_Struct()->m_cricket.m_RFP.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_LFS_Origin_X,MODEL_LFS_Origin_Y,MODEL_LFS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_LFS.m_Joint_Angle)*kMath::h_Trans(MODEL_LFP_Origin_X,MODEL_LFP_Origin_Y,MODEL_LFP_Origin_Z)*kMath::h_Roty(Sim_Struct()->m_cricket.m_LFP.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFP());
              glPopMatrix();
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_RRS_Origin_X,MODEL_RRS_Origin_Y,MODEL_RRS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_RRS.m_Joint_Angle)*kMath::h_Trans(MODEL_RRP_Origin_X,MODEL_RRP_Origin_Y,MODEL_RRP_Origin_Z)*kMath::h_Roty(Sim_Struct()->m_cricket.m_RRP.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFP());
              glPopMatrix();
      ...
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3,
      Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)*kMath::h_Trans(MODEL_LRS_Origin_X,MODEL_LRS_Origin_Y,MODEL_LRS_Origin_Z)*kMath::h_Rotx(Sim_Struct()->m_cricket.m_LRS.m_Joint_Angle)*kMath::h_Trans(MODEL_LRP_Origin_X,MODEL_LRP_Origin_Y,MODEL_LRP_Origin_Z)*kMath::h_Roty(Sim_Struct()->m_cricket.m_LRP.m_Joint_Angle);
              glMultMatrixf(Global_Trans.OGL());
              glCallList(kSim::drawList_RFP());
              glPopMatrix();
      }
      ...
      // Draw Simplified Tracks.
      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(0.250, -0.150, 0.0) * kMath::h_Roty(Sim_Struct()->m_cricket.m_RFT.m_Joint_Angle)
          * kMath::h_Scale(1.0, -1.0, 1.0);
        glMultMatrixf(Global_Trans.OGL());
        glCallList(kSim::drawList_RFT());
        glPopMatrix();
        ...
        Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(0.250, 0.150, 0.0) * kMath::h_Roty(Sim_Struct()->m_cricket.m_LFT.m_Joint_Angle);
        glMultMatrixf(Global_Trans.OGL());
        glCallList(kSim::drawList_RFT());
        glPopMatrix();
      }

      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(-0.250, -0.150, 0.0) * kMath::h_Roty(Sim_Struct()->m_cricket.m_RRT.m_Joint_Angle)
          * kMath::h_Scale(1.0, -1.0, 1.0);
        glMultMatrixf(Global_Trans.OGL());
        glCallList(kSim::drawList_RFT());
        glPopMatrix();
        ...
        Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(-0.250, 0.150, 0.0) * kMath::h_Roty(Sim_Struct()->m_cricket.m_LRT.m_Joint_Angle);
        glMultMatrixf(Global_Trans.OGL());
        glCallList(kSim::drawList_RFT());
        glPopMatrix();
      }
    ...
    case kSim::Sim_Struct::Contacts:
    {
      ...
      for (int i = 0; i < Sim_Struct()->m_track_contacts.get_size(); i++)
      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(MODEL_RFS_Origin_X, MODEL_RFS_Origin_Y, MODEL_RFS_Origin_Z)
          * kMath::h_Rotx(Sim_Struct()->m_cricket.m_RFS.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RFP_Origin_X, MODEL_RFP_Origin_Y, MODEL_RFP_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RFP.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RFD_Origin_X, MODEL_RFD_Origin_Y, MODEL_RFD_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RFD.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RFT_Origin_X, MODEL_RFT_Origin_Y, MODEL_RFT_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RFT.m_Joint_Angle)
          * kMath::h_Trans(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z)
          * kMath::h_Scale(MODEL_CONTACT_DIM);
        glMultMatrixf(Global_Trans.OGL());
        glCallList(kSim::drawList_Box());
        glPopMatrix();
        glBegin(GL_LINES);
        kMath::hVector Line_Start =
          kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                         Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          ...
          * kMath::h_Trans(MODEL_RFT_Origin_X, MODEL_RFT_Origin_Y, MODEL_RFT_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RFT.m_Joint_Angle)
          * kMath::h_Point(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z);
        glVertex3f(Line_Start(0), Line_Start(1), Line_Start(2));
        kMath::hVector Line_End =
          kMath::h_Trans(Sim_Struct()->m_cricket.m_BDY.m_Pos_X, Sim_Struct()->m_cricket.m_BDY.m_Pos_Y,
                         Sim_Struct()->m_cricket.m_BDY.m_Pos_Z)
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          ...
          * kMath::h_Trans(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z)
          * kMath::h_Scale(MODEL_CONTACT_NORM_LENGTH)
          * kMath::h_Point(Sim_Struct()->m_track_contacts(i).m_norm_x, Sim_Struct()->m_track_contacts(i).m_norm_y,
                           Sim_Struct()->m_track_contacts(i).m_norm_z);
      ...
      for (int i = 0; i < Sim_Struct()->m_track_contacts.get_size(); i++)
      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(MODEL_LFS_Origin_X, MODEL_LFS_Origin_Y, MODEL_LFS_Origin_Z)
          * kMath::h_Rotx(Sim_Struct()->m_cricket.m_LFS.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LFP_Origin_X, MODEL_LFP_Origin_Y, MODEL_LFP_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LFP.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LFD_Origin_X, MODEL_LFD_Origin_Y, MODEL_LFD_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LFD.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LFT_Origin_X, MODEL_LFT_Origin_Y, MODEL_LFT_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LFT.m_Joint_Angle)
          * kMath::h_Trans(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z)
          * kMath::h_Scale(MODEL_CONTACT_DIM);
        glMultMatrixf(Global_Trans.OGL());
      ...
      for (int i = 0; i < Sim_Struct()->m_track_contacts.get_size(); i++)
      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(MODEL_RRS_Origin_X, MODEL_RRS_Origin_Y, MODEL_RRS_Origin_Z)
          * kMath::h_Rotx(Sim_Struct()->m_cricket.m_RRS.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RRP_Origin_X, MODEL_RRP_Origin_Y, MODEL_RRP_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RRP.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RRD_Origin_X, MODEL_RRD_Origin_Y, MODEL_RRD_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RRD.m_Joint_Angle)
          * kMath::h_Trans(MODEL_RRT_Origin_X, MODEL_RRT_Origin_Y, MODEL_RRT_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_RRT.m_Joint_Angle)
          * kMath::h_Trans(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z)
          * kMath::h_Scale(MODEL_CONTACT_DIM);
        glMultMatrixf(Global_Trans.OGL());
      ...
      for (int i = 0; i < Sim_Struct()->m_track_contacts.get_size(); i++)
      {
        ...
        kMath::hMatrix Global_Trans =
          ...
          * kMath::h_RotQuat(Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat1, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat2,
                             Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat3, Sim_Struct()->m_cricket.m_BDY.m_Pos_Quat4)
          * kMath::h_Trans(MODEL_LRS_Origin_X, MODEL_LRS_Origin_Y, MODEL_LRS_Origin_Z)
          * kMath::h_Rotx(Sim_Struct()->m_cricket.m_LRS.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LRP_Origin_X, MODEL_LRP_Origin_Y, MODEL_LRP_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LRP.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LRD_Origin_X, MODEL_LRD_Origin_Y, MODEL_LRD_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LRD.m_Joint_Angle)
          * kMath::h_Trans(MODEL_LRT_Origin_X, MODEL_LRT_Origin_Y, MODEL_LRT_Origin_Z)
          * kMath::h_Roty(Sim_Struct()->m_cricket.m_LRT.m_Joint_Angle)
          * kMath::h_Trans(Sim_Struct()->m_track_contacts(i).m_pos_x, Sim_Struct()->m_track_contacts(i).m_pos_y,
                           Sim_Struct()->m_track_contacts(i).m_pos_z)
  ...
  if (!(Sim_Struct()->m_SR_image))
  {
    switch (key)
    {
      ...
      case 'W':
        ...
        if (Sim_Struct()->m_display_pitch < -0.45 * kSim::PI) Sim_Struct()->m_display_pitch = -0.45 * kSim::PI;
        glutPostRedisplay();
        break;
      case 'a':
      case 'A':
      ...
      case 'R':
        ...
        if (Sim_Struct()->m_display_zoom < 0.2) Sim_Struct()->m_display_zoom = 0.2;
        glutPostRedisplay();
        break;
      case 'f':
      case 'F':
        Sim_Struct()->m_display_zoom += 0.2;
        if (Sim_Struct()->m_display_zoom > 3.0) Sim_Struct()->m_display_zoom = 3.0;
        glutPostRedisplay();
        break;
      case 'y':
      case 'Y':
...
void simLoop()
{
  ...
  if (Global_Time <= 2.1)
  {
    // Process the hardware module (this may request an SR4000 image, which
    // necessitates bypassing other modules).
...
void KOS_Hardware_Handler(double Global_Time)
{
  ...
  if (Sim_Struct()->m_SR_image)
  {
    ...
    // Create new data message to Situational Awareness Module containing state
    // information.
