  ...
  if (m_inbox.check_message())
  {
    ...
    switch (temp->destination)
    {
      ...
      case KOS::e_Motion:
        ...
        break;
      default:
        std::cout << "\n\n Got a message that I don't know what to do with. - "
                     "Message_Hub\n";
...
void KOS::KOS_SubGrid::train(const int x,
                             const int y,
                             const int z,
                             const KOS::KOS_DEC sensor_conf,
                             const KOS::KOS_DEC sensed_occ)
{
  ...
  if (m_confidence[target_cell] > 0) m_confidence[target_cell] = 0;
}
...
KOS::KOS_DEC KOS::KOS_ExoGrid_X::get_data(const KOS::KOS_DEC x_pos, const int y, const int z)
{
  ...
  if (ExoGrid_Cell + m_zero_cell < 0 || ExoGrid_Cell + m_zero_cell >= m_cell_count) return Default_Occupancy;
...
void KOS::KOS_ExoGrid_X::train(const KOS::KOS_DEC x_pos,
                               const int y,
                               const int z,
                               const KOS::KOS_DEC sensor_conf,
                               const KOS::KOS_DEC sensed_occ)
...
void KOS::KOS_ExoGrid_Y::train(const KOS::KOS_DEC x_pos,
                               const KOS::KOS_DEC y_pos,
                               const int z,
                               const KOS::KOS_DEC sensor_conf,
                               const KOS::KOS_DEC sensed_occ)
{
...
void KOS::KOS_ExoGrid_Z::train(const KOS_DEC x_pos,
                               const KOS_DEC y_pos,
                               const KOS_DEC z_pos,
                               const KOS::KOS_DEC sensor_conf,
                               const KOS::KOS_DEC sensed_occ)
...
void KOS::KOS_Situational::process_message()
{
  ...
  if (m_inbox.check_message())
  {
    ...
    if (temp->message_type == "New State Frame")
    {
      ...
      // Generate a homogeneous transformation matrix from camera local space to
      // global space.
      kMath::hMatrix cam_to_global_trans;
      cam_to_global_trans = kMath::h_Trans(m_cricket.BDY_pos_x, m_cricket.BDY_pos_y, m_cricket.BDY_pos_z)
                            * kMath::h_Rotz(m_cricket.BDY_yaw) * kMath::h_Roty(m_cricket.BDY_pitch)
                            * kMath::h_Rotx(m_cricket.BDY_roll) * kMath::h_Trans(0.5 * 0.406, 0, 0)
                            * kMath::h_Roty(m_cricket.CAM);
      ...
      /*for(int i = 0; i < 176*144; i++) {
              if(image[i].mag() < 2.5) {
                      ...
                      for(double j = 0.5; j <= 1.05; j +=
      Dist_Per_Train_Poll/image[i].mag()) { double sensor_conf
      = 2.0*std::exp(-0.5*std::pow(j-1.0,2)*std::pow(0.04246610,-2))-1.0;
                              if(sensor_conf > 0)
                                      m_map.train(cam_to_global_trans*kMath::h_Scale(j)*image[i],
      sensor_conf, 1.0); else
                                      m_map.train(cam_to_global_trans*kMath::h_Scale(j)*image[i],
      -sensor_conf, 0.0);
                      }
              }
