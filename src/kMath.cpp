...
// This is the main constructor for kMatrix objects. It can be used with one or
// two inputs, if one input is used then an m x 1 vector is created.
kMath::kMatrix::kMatrix(const KOS_INT rows, const KOS_INT columns)
{
  if (rows <= 0 || columns <= 0) throw Range_Error();
  size_r = rows;
  size_c = columns;
...
// A basic display function, will output the matrix in text format to the main
// cout stream.
...
// Round bracket access operator. Allows users to access matrix values via
// Mat(row,col). This is a non-const operation so use allows the value at
// (row,col) to be modified.
kMath::KOS_DEC& kMath::kMatrix::operator()(KOS_INT row, KOS_INT col)
{
  if (row < 0 || row >= size_r || col < 0 || col >= size_c) throw Range_Error();
  return data[row][col];
}

// This is another round bracket access operator, but designed for use on
// vectors. If this is called on a non-vector (ie: column count greater than 1)
// this will throw a range error.
kMath::KOS_DEC& kMath::kMatrix::operator()(KOS_INT row)
{
  if (row < 0 || row >= size_r || size_c != 1) throw Range_Error();
  return data[row][0];
}
...
void kMath::kMatrix::rref()
{
  ...
  // The row_counter will track how far down the matrix we've progressed, the
  // lead counter will track how far right in the matrix we've progressed. If
  // either of these reach their respective maximums, we've finished.
  for (int lead = 0, row_counter = 0; lead < size_c && row_counter < size_r; lead++, row_counter++)
  {
    ...
    while (row_counter < size_r)
    {
      ...
      {
        if (std::abs(data[j][lead]) > std::abs(data[i][lead])) i = j;
      }

      // If the largest leading term we found was zero, we know that we can't do
      // anything to this column, so increment the lead counter to the next
      // column.
      if (data[i][lead] == 0)
      {
        lead++;
...
void kMath::kMatrix::clean(int x)
{
  for (int j = 0; j < size_r; j++)
    for (int k = 0; k < size_c; k++)
      if (std::abs(data[j][k]) < 1.0 / pow(10, x)) data[j][k] = 0;
}

// Assignment operator, checks for self assignment and performs proper memory
...
{
  if (arg1.get_size_r() != arg2.get_size_r() || arg1.get_size_c() != arg2.get_size_c()) throw Range_Error();
  kMatrix ret_val(arg1.get_size_r(), arg1.get_size_c());
  for (int i = 0; i < ret_val.get_size_r(); i++)
  {
    for (int j = 0; j < ret_val.get_size_c(); j++)
      ret_val(i, j) = arg1.get_val(i, j) - arg2.get_val(i, j);
  }
  return ret_val;
}
...
kMatrix operator+(const kMatrix& arg1, const kMatrix& arg2)
{
  ...
  {
    std::cerr << "kMatrix::Operator+() Failed size test. (" << arg1.get_size_r() << "x" << arg1.get_size_c() << "), ("
              << arg2.get_size_r() << "x" << arg2.get_size_c() << ").\n";
    throw Range_Error();
  }
  kMatrix ret_val(arg1.get_size_r(), arg1.get_size_c());
  for (int i = 0; i < ret_val.get_size_r(); i++)
...
kMatrix operator*(const kMatrix& arg1, const kMatrix& arg2)
{
  if (arg1.get_size_c() != arg2.get_size_r()) throw Range_Error();
  ...
  for (int i = 0; i < ret_val.get_size_r(); i++)
  {
    ...
    {
      ret_val(i, j) = 0;
      for (int k = 0; k < arg1.get_size_c(); k++)
        ret_val(i, j) += arg1.get_val(i, k) * arg2.get_val(k, j);
    }
  }
  return ret_val;
}
...
kMatrix operator*(const KOS_DEC& arg1, const kMatrix& arg2)
{
  ...
  {
    for (int j = 0; j < ret_val.get_size_c(); j++)
      ret_val(i, j) = arg1 * arg2.get_val(i, j);
  }
  return ret_val;
}
...
kMatrix operator*(const kMatrix& arg1, const KOS_DEC& arg2)
{
  ...
  {
    for (int j = 0; j < ret_val.get_size_c(); j++)
      ret_val(i, j) = arg2 * arg1.get_val(i, j);
  }
  return ret_val;
}
...
{
  if (GL_Table != NULL) delete[] GL_Table;
...
{
  if (arg1.get_size_r() != 4 || arg1.get_size_c() != 4) throw Range_Error();
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
      data[i][j] = arg1.get_val(i, j);
  ...
  if (this != &arg1)
  {
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        data[i][j] = 0;
        for (int k = 0; k < 4; k++)
          data[i][j] += arg1.get_val(i, k) * temp.get_val(k, j);
      }
    }
  }
  else
  {
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        data[i][j] = 0;
        for (int k = 0; k < 4; k++)
          data[i][j] += temp.get_val(i, k) * temp.get_val(k, j);
      }
    }
  }
  return *this;
...
{
  if (arg1.get_size_r() != 4 || arg1.get_size_c() != 1) throw Range_Error();
  for (int i = 0; i < 4; i++)
  {
...
kMath::hVector& kMath::hVector::operator*=(const hMatrix& arg1)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    data[i] = 0;
    for (int k = 0; k < 4; k++)
      data[i] += arg1.get_val(i, k) * temp.get_val(k);
  }
  return *this;
}
...
hVector operator*(const hMatrix& arg1, const hVector& arg2)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ret_val(i) = 0;
    for (int k = 0; k < 4; k++)
      ret_val(i) += arg1.get_val(i, k) * arg2.get_val(k);
  }
  return ret_val;
}
...
{
  return arg_1.get_val(0) * arg_2.get_val(0) + arg_1.get_val(1) * arg_2.get_val(1)
         + arg_1.get_val(2) * arg_2.get_val(2);
}
...
hVector h_VRotx(const KOS_DEC& Angle_Pos, const KOS_DEC& Angle_Vel, const hVector& Pos, const hVector& Vel)
{
  ...
  ret_val(1) = cos(Angle_Pos) * Vel.get_val(1) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(1)
               - sin(Angle_Pos) * Vel.get_val(2) - cos(Angle_Pos) * Angle_Vel * Pos.get_val(2);
  ret_val(2) = sin(Angle_Pos) * Vel.get_val(1) + cos(Angle_Pos) * Angle_Vel * Pos.get_val(1)
               + cos(Angle_Pos) * Vel.get_val(2) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(2);
  ret_val(3) = 0;
  return ret_val;
}
...
{
  hVector ret_val;
  ret_val(0) = cos(Angle_Pos) * Vel.get_val(0) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(0)
               + sin(Angle_Pos) * Vel.get_val(2) + cos(Angle_Pos) * Angle_Vel * Pos.get_val(2);
  ret_val(1) = Vel.get_val(1);
  ret_val(2) = -sin(Angle_Pos) * Vel.get_val(0) - cos(Angle_Pos) * Angle_Vel * Pos.get_val(0)
               + cos(Angle_Pos) * Vel.get_val(2) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(2);
  ret_val(3) = 0;
  return ret_val;
}
...
hVector h_VRotz(const KOS_DEC& Angle_Pos, const KOS_DEC& Angle_Vel, const hVector& Pos, const hVector& Vel)
{
  hVector ret_val;
  ret_val(0) = cos(Angle_Pos) * Vel.get_val(0) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(0)
               - sin(Angle_Pos) * Vel.get_val(1) - cos(Angle_Pos) * Angle_Vel * Pos.get_val(1);
  ret_val(1) = sin(Angle_Pos) * Vel.get_val(0) + cos(Angle_Pos) * Angle_Vel * Pos.get_val(0)
               + cos(Angle_Pos) * Vel.get_val(1) - sin(Angle_Pos) * Angle_Vel * Pos.get_val(1);
  ret_val(2) = Vel.get_val(2);
  ret_val(3) = 0;
  return ret_val;
}

hVector h_VRotQuat(const KOS_DEC& QuatA_Pos,
                   const KOS_DEC& QuatB_Pos,
                   const KOS_DEC& QuatC_Pos,
                   const KOS_DEC& QuatD_Pos,
                   const KOS_DEC& QuatA_Vel,
                   const KOS_DEC& QuatB_Vel,
                   const KOS_DEC& QuatC_Vel,
                   const KOS_DEC& QuatD_Vel,
                   const hVector& Pos,
                   const hVector& Vel)
{
  hVector ret_val;
  ret_val(0) =
    (1 - 2 * pow(QuatC_Pos, 2) - 2 * pow(QuatD_Pos, 2)) * Vel.get_val(0)
    + (2 * QuatB_Pos * QuatC_Pos - 2 * QuatA_Pos * QuatD_Pos) * Vel.get_val(1)
    + (2 * QuatB_Pos * QuatD_Pos + 2 * QuatA_Pos * QuatC_Pos) * Vel.get_val(2)
    + (-4 * QuatC_Pos * QuatC_Vel - 4 * QuatD_Pos * QuatD_Vel) * Pos.get_val(0)
    + (2 * QuatB_Vel * QuatC_Pos + 2 * QuatB_Pos * QuatC_Vel - 2 * QuatA_Vel * QuatD_Pos - 2 * QuatA_Pos * QuatD_Vel)
        * Pos.get_val(1)
    + (2 * QuatB_Vel * QuatD_Pos + 2 * QuatB_Pos * QuatD_Vel + 2 * QuatA_Vel * QuatC_Pos + 2 * QuatA_Pos * QuatC_Vel)
        * Pos.get_val(2);
  ret_val(1) =
    (2 * QuatB_Pos * QuatC_Pos + 2 * QuatA_Pos * QuatD_Pos) * Vel.get_val(0)
    + (1 - 2 * pow(QuatB_Pos, 2) - 2 * pow(QuatD_Pos, 2)) * Vel.get_val(1)
    + (2 * QuatC_Pos * QuatD_Pos - 2 * QuatA_Pos * QuatB_Pos) * Vel.get_val(2)
    + (2 * QuatB_Vel * QuatC_Pos + 2 * QuatB_Pos * QuatC_Vel + 2 * QuatA_Vel * QuatD_Pos + 2 * QuatA_Pos * QuatD_Vel)
        * Pos.get_val(0)
    + (-4 * QuatB_Pos * QuatB_Vel - 4 * QuatD_Pos * QuatD_Vel) * Pos.get_val(1)
    + (2 * QuatC_Vel * QuatD_Pos + 2 * QuatC_Pos * QuatD_Vel - 2 * QuatA_Vel * QuatB_Pos - 2 * QuatA_Pos * QuatB_Vel)
        * Pos.get_val(2);
  ret_val(2) =
    (2 * QuatB_Pos * QuatD_Pos - 2 * QuatA_Pos * QuatC_Pos) * Vel.get_val(0)
    + (2 * QuatC_Pos * QuatD_Pos + 2 * QuatA_Pos * QuatB_Pos) * Vel.get_val(1)
    + (1 - 2 * pow(QuatB_Pos, 2) - 2 * pow(QuatC_Pos, 2)) * Vel.get_val(2)
    + (2 * QuatB_Vel * QuatD_Pos + 2 * QuatB_Pos * QuatD_Vel - 2 * QuatA_Vel * QuatC_Pos - 2 * QuatA_Pos * QuatC_Vel)
        * Pos.get_val(0)
    + (2 * QuatC_Vel * QuatD_Pos + 2 * QuatC_Pos * QuatD_Vel + 2 * QuatA_Vel * QuatB_Pos + 2 * QuatA_Pos * QuatB_Vel)
        * Pos.get_val(1)
    + (-4 * QuatB_Pos * QuatB_Vel - 4 * QuatC_Pos * QuatC_Vel) * Pos.get_val(2);
  ret_val(3) = 0;
  return ret_val;
}
